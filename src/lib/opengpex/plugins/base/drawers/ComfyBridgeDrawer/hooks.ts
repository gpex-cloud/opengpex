...
export function useComfyBridgeState() {
  ...
  // ─── Test Connection Handler (+ auto-sync object_info on success) ──────────
  ...
  // On successful connection, automatically syncs /object_info if there's an active workflow.
  const handleTestConnection = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      // Auto-sync object_info on successful connection (non-blocking)
      if (success && activeWorkflow && activeWorkflow.exposedParams.length > 0) {
        handleSyncObjectInfo().catch(() => { /* ignore sync errors during refresh */ });
      }
  ...
  }, [testConnectionCmd, connectionStatus, activeWorkflow, handleSyncObjectInfo]);
