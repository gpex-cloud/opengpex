...
import type { BgRemovalRequest, BgRemovalResponse, BgRemovalResult, BgRemovalProgress } from './protocol';

/**
 * BgRemovalClient — Singleton wrapper around the BgRemoval Worker.
 *
...
export class BgRemovalClient {
   ...
   * @returns Promise resolving with the final BgRemovalResult
   */
  run(
    req: Omit<BgRemovalRequest, 'reqId'>,
    opts?: {
      signal?: AbortSignal;
      timeoutMs?: number;
    ...
    return new Promise<BgRemovalResult>((resolve, reject) => {
      ...
      const onMessage = (ev: MessageEvent<BgRemovalResponse>) => {
        const msg = ev.data;
        ...
        switch (msg.type) {
          ...
          case 'error':
            ...
            // sufficient and avoids unnecessary model re-downloads.
            {
              ...
              if (isWebGpuOrtError) {
                console.warn('[BgRemovalClient] WebGPU ORT error detected — disposing worker for clean restart');
                this.dispose();
              }
            }
            reject(new Error(`BgRemoval worker error: ${msg.error}`));
            break;
        }
      };
      ...
      const onError = (ev: ErrorEvent) => {
        cleanup();
        this.dispose();
        reject(new Error(`BgRemoval worker crashed: ${ev.message ?? 'unknown error'}`));
      };
      ...
      const timer = timeoutMs > 0
        ? setTimeout(() => {
            cleanup();
            this.dispose();
            reject(new Error(`BgRemoval timed out after ${timeoutMs}ms`));
          }, timeoutMs)
        : null;
...
/**
 * Module-level singleton — one BgRemoval worker per editor session.
 * The model stays warm in GPU/WASM memory for instant subsequent invocations.
 */
export const bgRemovalClient = new BgRemovalClient();
