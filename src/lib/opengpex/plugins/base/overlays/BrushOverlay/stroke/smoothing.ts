 ...
 * - Fixed-capacity window avoids array slice/allocation
 * - Pre-sized result array hint reduces GC pressure
 */
...
export function catmullRomInterpolate(
  ...
  const result: Point2D[] = new Array(numSegments);
 ...
 * Uses a fixed-capacity ring buffer approach to avoid array reallocation.
 *
 * Usage:
...
export class StrokeSmoother {
  ...
  addPoint(point: Point2D): Point2D[] {
    ...
    // Keep window size bounded (avoid unbounded growth)
