...
/**
 * normalizer.ts — Layer state → FilterDescriptor[] conversion + cache key.
 *
 * Migrated from v1 `engine/filters/normalizer.ts` with imports updated to
 * use v2's `protocol/IFilter` type definitions.
 *
 * Purpose (spec §5.5.1):
 ...
 * kept in-order (their order is meaningful).
 */
