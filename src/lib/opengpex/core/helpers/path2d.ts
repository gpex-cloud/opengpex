...
import { smoothToStairedPath } from '@opengpex/editor/core/geometry/operators/point2d';
...
export function shapeToPath2D(shape: Shape): Path2D {
  ...
  if (type === 'rect') {
    ...
    if (antiAliased === false) {
      const stairedD = smoothToStairedPath(shape.pathData);
...
//
// The `type:'path'` stair-step transform (`smoothToStairedPath`) and its shared
// Bresenham helper now live in the geometry engine:
//   - `core/geometry/operators/point2d.ts::smoothToStairedPath`
//   - `core/geometry/bresenham.ts::bresenhamSteps`
// `shapeToPath2D` (above) imports `smoothToStairedPath` for render-time No-AA routing.
//
// The `polygonToShape` serializer (pure Polygon → Shape, no DOM dependency) now
// lives in `core/geometry/operators/polygon.ts`.
