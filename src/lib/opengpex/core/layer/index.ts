...
import { GeometryService, PixelService, AssetService, LayerService, LayerUpdateTx, LayerEditor, asLocalShape, asLocalRect, asWorldShape, EditorActions, EditorData, Layer, VectorMask, BitmapMask } from '@opengpex/editor/core/types';
...
export function createLayerService(
  ...
  return {
    ...
    fragmentToLayerLogical: (frame, layer, nameType) => {
      const localShape = geometry.shape.frameLocalToLayerLocal(frame.imageCropBox, frame, layer);
    ...
    fragmentToExistLayer: (frame, sourceLayer, targetLayer) => {
      const localShape = geometry.shape.frameLocalToLayerLocal(frame.imageCropBox, frame, sourceLayer);
    ...
    fragmentToLayerPhysical: async (frame, layer, nameType) => {
      ...
      const selection = geometry.shape.localToWorldShape(frame.imageCropBox, frame);
      const targetW = Math.round(selection.rect.w);
      const targetH = Math.round(selection.rect.h);

      const assetResult = await pixels.worker.asAsset(
        pixels.worker.mergeLayersWithShape([layer], selection)
      );
      ...
      const cx = selection.rect.x + (trimmedRect.x + trimmedRect.w / 2);
      const cy = selection.rect.y + (trimmedRect.y + trimmedRect.h / 2);
      ...
      const newLayer = LayerFactory.getNewLayer({
        ...
        name: LayerFactory.getNewLayerName(frame.layers.order.map(id => frame.layers.byId[id]), nameType),
        parentId: undefined,
        visibleShape: asLocalShape({ x: 0, y: 0, w: trimmedRect.w, h: trimmedRect.h }, selection.type),
