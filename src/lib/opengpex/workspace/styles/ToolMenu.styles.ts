 ...
 */
export const getToolMenuStyles = (isCollapsed: boolean) => {
    return {
        // Main shell (floating mode)
        container: {
            className: `
        ...
        menuItem: {
            button: `
                group relative flex items-center w-full h-[30px] px-3 gap-3 rounded-lg
            ...
            label: "flex-1 text-[12px] font-medium text-[var(--text-main)] normal-case whitespace-nowrap",
            shortcut: "text-[10px] font-black text-[var(--text-muted)] uppercase tracking-tighter opacity-60"
        },
